...
$ vagrant ssh injector-controller --command "cd /home/vagrant/perftest-toolkit/deployment;ansible-playbook -i /vagrant/3scale.csv -i /vagrant/injector-cluster.hosts /home/vagrant/perftest-toolkit/deployment/injector.yml -e injector_target_port=8099 -e csv_file_path=/vagrant -e ansible_hostname=192.168.42.45 "
...
The above commands starts up the cluster in sequential order. Provisioning a base platform with the latest security updates for the release. Ansible is used to set up the load driver and start a test run. Ansible uses an inventory plugin to read a CSV file.
 This CSV file is expected to be generated by Buddhi.
 In this scenario we are testing the injector driver to push load at API services. Without 3Scale AMP. The apitarget cluster mocks the services 3Scale AMP establishes. Instead using hostnames in the CSV to configure Hyperfoil drivers.
